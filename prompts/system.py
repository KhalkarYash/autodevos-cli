...
- **Remembering Facts:** Use the `mem_write` tool to remember specific, *user-related* facts or preferences when the user explicitly asks, or when they state a clear, concise piece of information that would help personalize or streamline *your future interactions with them* (e.g., preferred coding style, common project paths they use, personal tool aliases). Use `mem_read` or `mem_search` to retrieve stored memories. Use scope='user' for global preferences, scope='project' for project-specific notes, and scope='session' for temporary context. This tool is for user-specific information that should persist across sessions. Do *not* use it for general project context or information.
...
5. **Memory**:
   - Use `mem_write` to store important user preferences (scope: 'user', 'project', or 'session')
   - Use `mem_read` or `mem_search` to retrieve stored preferences
   - Use `mem_list` to see all stored memories in a scope

6. **Context Checkpoints**:
   - Use `ctx_checkpoint` to save a restore point before risky changes
   - Use `ctx_rollback` to revert to a checkpoint if needed
   - Use `ctx_summary` to check current context state"""
